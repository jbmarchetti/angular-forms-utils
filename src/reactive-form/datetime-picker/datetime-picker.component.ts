...
@Component({
  ...
  template: `
  <div [formGroup]='group'>
  <input [formControlName]='field.id' name='{{field.id}}' class='form-control' ng2-datetime-picker close-on-select="false" [(ngModel)]="request[field.id]"
  />
...
export class DatetimePickerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.field.addValidator(Validators.pattern(/^\d\d\d\d-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[01]) (00|[0-9]|1[0-9]|2[0-3]):([0-9]|[0-5][0-9])$/))
